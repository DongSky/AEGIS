...
- **Reference Images**: <ref_images_placeholder_list>
- **Output Image**: <output_images_placeholder_list>
- **Checklist**: {checklist}
...
def replace_image_tags(match):
    ...
    return f"<ref_image{number}>"


def replace_output_image_tags(match):
    return '<output_image>'

def replace_checklist_image_tags(match):
    number = match.group(1)
    if number == '1':
...
def process_item(item):
    ...
    output_image_path_list = item['output_image_path_list']
    ...
    pattern = r"(<image\d+>|<ref_images_placeholder_list>|<output_images_placeholder_list>|<ref_image\d+>|<output_image\d+>|<output_image>)"
    ...
    if "<image" in output_text:
        ...
        cur_pattern = r"<image>"
        output_text = re.sub(cur_pattern, replace_output_image_tags, output_text)

    for i, item in enumerate(checklist):
        if "<ref_image" in item or "<output_image" in item:
            cur_pattern = r"<ref_image(\d+)>|<output_image(\d+)>|<image(\d+)>"
            item = re.sub(cur_pattern, replace_checklist_image_tags, item)
            checklist[i] = item


    parts = re.split(pattern,  eval_prompt.format(clear_prompt=clear_prompt, task_type=task_type, category=category, checklist=checklist, reference_text=ref_text, output_text=output_text))
    ...
    for part in parts:
        ...
        elif re.match(r'<output_images_placeholder_list>', part):
            prompt_part_list.append('<output_images_placeholder_list>')
        elif re.match(r'<ref_image\d+>', part):
            prompt_part_list.append('<ref_image>')
        elif re.match(r'<output_image\d+>', part):
            prompt_part_list.append('<output_image>')
        elif re.match(r'<output_image>', part):
            prompt_part_list.append('<output_image>')
        else:
    ...
    try:
        for part in prompt_part_list:
            ...
            if part == '<input_image>':
                ...
                image_p = ref_images[ref_image_id]
                full_image_path = os.path.join(image_dir, image_p)
                cur_image = Image.open(full_image_path)
                input_data.append(cur_image)
                ref_image_id += 1
            elif part == '<output_image>':
                image_p = output_image_path_list[output_image_id]
                full_image_path = os.path.join(root_dir, image_p)
                cur_image = Image.open(full_image_path)
                input_data.append(cur_image)
                output_image_id += 1
                ...
                for image_p in ref_images:
                    ...
                    cur_image = Image.open(full_image_path)
                    input_data.append(cur_image)
            elif part == '<output_images_placeholder_list>':
                for image_p in output_image_path_list:
                    cur_image = Image.open(image_p)
                    input_data.append(cur_image)
            else:
    ...
    if not is_response_successful:
        output_text = f'response failed for {cur_id}'
        to_write_item['output_text'] = output_text
        to_write_item['answer_list'] = output_text
        to_write_item['reason_list'] = output_text
        to_write_item['eval_model_name'] = global_model_name
...
def main():
    ...
    sub_output_dir = f'{output_dir}/interleave_eval'
